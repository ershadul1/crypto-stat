...
const fetchList = filter => dispatch => axios.get(
  `${URL}/markets?vs_currency=${filter.currency}&order=market_cap_desc&per_page=100&page=1&sparkline=false&price_change_percentage=24h%2C7d`,
)
  .then(response => {
...
const fetchDetails = id => dispatch => axios.get(
  `${URL}/${id}?localization=false&tickers=false&market_data=true&community_data=false&developer_data=false&sparkline=true`,
)
  .then(response => {
